...
class OpCoreGUI(FluentWindow):
    ...
    def init_navigation(self):
        ...
        self.addSubInterface(self.uploadPage, FluentIcon.FOLDER_ADD, "1. Upload Report", NavigationItemPosition.TOP)
        self.addSubInterface(self.compatibilityPage, FluentIcon.SEARCH, "2. Compatibility", NavigationItemPosition.TOP)
        self.addSubInterface(self.configurationPage, FluentIcon.SETTING, "3. Configuration", NavigationItemPosition.TOP)
        self.addSubInterface(self.buildPage, FluentIcon.DEVELOPER_TOOLS, "4. Build EFI", NavigationItemPosition.TOP)
    ...
    def auto_select_macos_version(self):
        ...
        self.macos_version_text = os_data.get_macos_name_by_darwin(suggested_macos_version)
    ...
    def apply_macos_version(self, version, defer_kext_selection=False):
        ...
        # Select SMBIOS model
        self.smbios_model_text = self.ocpe.s.select_smbios_model(self.customized_hardware, version)
        ...
        if self.disabled_devices:
            self.disabled_devices_text = ", ".join(self.disabled_devices.keys())
        else:
            self.disabled_devices_text = "None"
    ...
    def export_hardware_report(self):
        ...
        def export_thread():
            ...
            if output[-1] != 0:
                ...
                # Load the report
                QTimer.singleShot(0, lambda: self.load_hardware_report(report_path, report_data))
    ...
    def build_efi(self):
        ...
        if not self.customized_hardware:
            self.update_status("Please load a hardware report first", 'warning')
            return
        ...
        def build_thread():
            try:
                ...
                self.ocpe.o.gather_bootloader_kexts(self.ocpe.k.kexts, self.macos_version_text)
                ...
                self.update_status("OpenCore EFI built successfully!", 'success')
