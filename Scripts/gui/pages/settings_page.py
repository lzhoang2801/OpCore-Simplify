...
class SettingsPage(QWidget):
    ...
    def create_build_settings_card(self):
        ...
        dir_desc = BodyLabel(
            "Choose where to save built EFI folders. Leave empty to use temporary directory.")
        dir_desc.setStyleSheet(
        ...
        self.output_dir_input.textChanged.connect(
            lambda text: self.settings.set("build_output_directory", text))
        ...
        self.open_folder_switch.checkedChanged.connect(
        ...
        self.clean_temp_switch.setChecked(
            self.settings.get("clean_temp_files_on_exit", True))
        self.clean_temp_switch.checkedChanged.connect(
    ...
    def create_boot_args_card(self):
        ...
        self.verbose_boot_switch.setChecked(
            self.settings.get("verbose_boot", True))
        self.verbose_boot_switch.checkedChanged.connect(
    ...
    def create_macos_version_card(self):
        ...
        self.include_beta_switch.setChecked(
            self.settings.get("include_beta_versions", False))
        self.include_beta_switch.checkedChanged.connect(
        ...
        pref_desc.setStyleSheet(
            f"color: {COLORS['text_secondary']}; font-size: 12px;")
        card_layout.addWidget(pref_desc)

        self.preferred_version_input = LineEdit(self)
        self.preferred_version_input.setPlaceholderText(
            "e.g., 23.0.0 (Ventura) - leave empty for auto")
        self.preferred_version_input.setText(
            self.settings.get("preferred_macos_version", ""))
    ...
    def create_boot_picker_card(self):
        ...
        self.show_picker_switch.setChecked(
            self.settings.get("show_picker", True))
        self.show_picker_switch.checkedChanged.connect(
        ...
        self.hide_aux_switch.setChecked(
            self.settings.get("hide_auxiliary", False))
        self.hide_aux_switch.checkedChanged.connect(
    ...
    def create_security_card(self):
        ...
        warning_desc = BodyLabel(
            "Modifying these settings may affect system security and stability.")
        warning_desc.setStyleSheet(
        ...
        self.disable_sip_switch.setChecked(
            self.settings.get("disable_sip", True))
        self.disable_sip_switch.checkedChanged.connect(
    ...
    def create_smbios_card(self):
        ...
        warning_desc = BodyLabel(
            "Custom SMBIOS values may affect iServices functionality. Use with caution.")
        warning_desc.setStyleSheet(
            "color: #FFA500; font-size: 12px; font-style: italic;")
        card_layout.addWidget(warning_desc)
        ...
        self.preserve_smbios_switch.setChecked(
            self.settings.get("preserve_smbios", False))
        self.preserve_smbios_switch.checkedChanged.connect(
        ...
        preserve_desc = BodyLabel(
            "Keep the same SMBIOS values across multiple builds.")
        preserve_desc.setStyleSheet(
            f"color: {COLORS['text_secondary']}; font-size: 12px;")
    ...
    def create_update_settings_card(self):
        ...
        self.auto_update_switch.setChecked(
        ...
        self.verify_integrity_switch.setChecked(
            self.settings.get("verify_download_integrity", True))
        self.verify_integrity_switch.checkedChanged.connect(
        ...
        self.force_redownload_switch.setChecked(
            self.settings.get("force_redownload", False))
        self.force_redownload_switch.checkedChanged.connect(
    ...
    def create_advanced_card(self):
        ...
        warning_desc = BodyLabel(
            "These settings are for advanced users only. Incorrect values may cause boot failures.")
        warning_desc.setStyleSheet(
            "color: #FF4444; font-size: 12px; font-style: italic;")
        card_layout.addWidget(warning_desc)
        ...
        self.debug_logging_switch.setChecked(
            self.settings.get("enable_debug_logging", False))
        self.debug_logging_switch.checkedChanged.connect(
        ...
        self.skip_acpi_switch.setChecked(
            self.settings.get("skip_acpi_validation", False))
        self.skip_acpi_switch.checkedChanged.connect(
        ...
        self.force_kext_switch.setChecked(
            self.settings.get("force_load_incompatible_kexts", False))
        self.force_kext_switch.checkedChanged.connect(
