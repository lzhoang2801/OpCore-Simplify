...
from Scripts.datasets import codec_layouts, gpu_data, os_data, pci_data
...
class CompatibilityChecker:
    ...
    def show_macos_compatibility(self, device_compatibility):
        ...
        if max_compatibility == min_version:
            return "\033[1;36mMaximum support up to {}\033[0m".format(os_data.get_macos_name_by_darwin(device_compatibility[-1]))

        if min_version < min_compatibility or max_compatibility < max_version:
            return "\033[1;32m{} to {}\033[0m".format(
                os_data.get_macos_name_by_darwin(device_compatibility[-1]), os_data.get_macos_name_by_darwin(device_compatibility[0])
            )

        return "\033[1;36mUp to {}\033[0m".format(os_data.get_macos_name_by_darwin(device_compatibility[0]))
    ...
    def check_cpu_compatibility(self):
        ...
        print(
            "{}- {}: {}".format(
                " " * 3,
                self.hardware_report.get("CPU").get("Processor Name"),
                self.show_macos_compatibility(self.hardware_report["CPU"].get("Compatibility")),
            )
        )
    ...
    def check_gpu_compatibility(self):
        ...
        for gpu_name, gpu_props in self.hardware_report["GPU"].items():
            ...
            if "Intel" in gpu_manufacturer:
                ...
                elif device_id.startswith("01") and device_id[-2] not in ("5", "6") and device_id not in ("0102", "0106", "010A"):
                    max_version = "17.99.99"
                elif device_id.startswith("01") and device_id not in ("0152", "0156"):
                    max_version = "20.99.99"
                elif device_id.startswith(("04", "0A", "0C", "0D", "0B", "16")):
                    max_version = "21.99.99"
                elif device_id.startswith(("09", "19", "59", "3E", "87", "9B")) and device_id not in (
                    "3E90",
                    "3E93",
                    "3E99",
                    "3E9C",
                ...
                if "Navi 2" in gpu_codename:
                    if "AVX2" not in self.hardware_report.get("CPU").get("SIMD Features"):
                        max_version = "21.99.99"
            ...
            if (max_version is None and min_version is None) or (
                ...
                if max_version and self.utils.parse_darwin_version(max_version) < self.utils.parse_darwin_version(ocl_patched_max_version):
                    gpu_props["OCLP Compatibility"] = (
                        ...
                        if self.utils.parse_darwin_version(ocl_patched_min_version)
                        > self.utils.parse_darwin_version("{}.{}.{}".format(int(max_version[:2]) + 1, 0, 0))
                        else "{}.{}.{}".format(int(max_version[:2]) + 1, 0, 0),
                    )
        ...
        for gpu_name, gpu_props in self.hardware_report.get("GPU").items():
            if gpu_props.get("Compatibility") != (None, None):
                if all(
                    other_gpu_props.get("Compatibility") == (None, None)
                    for other_gpu_props in self.hardware_report.get("GPU").values()
                    if other_gpu_props != gpu_props
                ):
                    pass
                elif any(
                    monitor_info.get("Connected GPU", gpu_name) != gpu_name
                    for monitor_info in self.hardware_report.get("Monitor", {}).values()
                    if monitor_info.get("Connector Type") == "Internal"
                ):
                    gpu_props["Compatibility"] = (None, None)
                ...
                max_supported_gpu_version = (
                    max_version
                    if not max_supported_gpu_version
                    else max_version
                    if self.utils.parse_darwin_version(max_version) > self.utils.parse_darwin_version(max_supported_gpu_version)
                    else max_supported_gpu_version
                )
                min_supported_gpu_version = (
                    min_version
                    if not min_supported_gpu_version
                    else min_version
                    if self.utils.parse_darwin_version(min_version) < self.utils.parse_darwin_version(min_supported_gpu_version)
                    else min_supported_gpu_version
                )

            if gpu_props.get("OCLP Compatibility"):
                self.ocl_patched_macos_version = (
                    gpu_props.get("OCLP Compatibility")[0],
                    self.ocl_patched_macos_version[-1]
                    if self.ocl_patched_macos_version
                    and self.utils.parse_darwin_version(self.ocl_patched_macos_version[-1])
                    < self.utils.parse_darwin_version(gpu_props.get("OCLP Compatibility")[-1])
        ...
        self.max_native_macos_version = (
            max_supported_gpu_version
            if self.utils.parse_darwin_version(max_supported_gpu_version) < self.utils.parse_darwin_version(self.max_native_macos_version)
            else self.max_native_macos_version
        )
        self.min_native_macos_version = (
            min_supported_gpu_version
            if self.utils.parse_darwin_version(min_supported_gpu_version) > self.utils.parse_darwin_version(self.min_native_macos_version)
            else self.min_native_macos_version
        )
    ...
    def check_sound_compatibility(self):
        for audio_device, audio_props in self.hardware_report.get("Sound", {}).items():
            ...
            if (
                "USB" in audio_props.get("Bus Type")
                or codec_id.startswith("1002")
                or codec_id.startswith("8086")
                and codec_id not in pci_data.IntelSSTIDs
                or codec_id in codec_layouts.data
            ):
    ...
    def check_network_compatibility(self):
        for device_name, device_props in self.hardware_report.get("Network", {}).items():
            ...
            if device_id in pci_data.WirelessCardIDs:
                if device_id not in pci_data.IntelWiFiIDs and device_id not in pci_data.AtherosWiFiIDs[8:]:
                    ...
                    self.ocl_patched_macos_version = (
                        ...
                        if self.ocl_patched_macos_version
                        and self.utils.parse_darwin_version(self.ocl_patched_macos_version[-1])
                        < self.utils.parse_darwin_version(device_props.get("OCLP Compatibility")[-1])
                        else device_props.get("OCLP Compatibility")[-1],
                    )
            ...
            if device_id in pci_data.WirelessCardIDs:
                if device_id in pci_data.BroadcomWiFiIDs:
                    print(
                        "{}- Continuity Support: \033[1;32mFull\033[0m (AirDrop, Handoff, Universal Clipboard, Instant Hotspot,...)".format(" " * 6)
                    )
    ...
    def check_storage_compatibility(self):
        ...
        if all(
            controller_props.get("Compatibility") == (None, None)
            for controller_name, controller_props in self.hardware_report["Storage Controllers"].items()
        ):
            print("")
