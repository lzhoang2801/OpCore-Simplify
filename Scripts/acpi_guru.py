...
from Scripts import dsdt, run, smbios, utils
from Scripts.datasets import acpi_patch_data, chipset_data, cpu_data, pci_data
...
class ACPIGuru:
    ...
    def get_unique_device(self, _path, base_name, starting_number=0, used_names=None):
        ...
        while True:
            ...
            if not len(self.acpi.get_device_paths("." + name)) and name not in used_names:
    ...
    def read_acpi_tables(self, path):
        ...
        if os.path.isdir(path):
            ...
            for t in self.sorted_nicely(os.listdir(path)):
                if "Patched" not in t and self.acpi.table_is_valid(path, t):
                    print(" - {}".format(t))
                    tables.append(t)
            if not tables:
        ...
        elif "Patched" not in path and os.path.isfile(path):
        ...
        if trouble_dsdt:
            ...
            for p in self.pre_patches:
                if not all(x in p for x in ("PrePatch", "Comment", "Find", "Replace")):
                    continue
        ...
        if not loaded_tables or failed:
            print("\nFailed to load tables in {}{}\n".format(os.path.dirname(path) if os.path.isfile(path) else path, ":" if failed else ""))
    ...
    def get_sta_var(self, var="STAS", device=None, dev_hid="ACPI000E", dev_name="AWAC", log_locate=False, table=None):
        ...
        if device:
            ...
            if not len(dev_list):
                if log_locate:
                    print(" - Could not locate {}".format(device))
                return {"value": False}
        else:
            if log_locate:
    ...
    def enable_cpu_power_management(self):
        ...
        for table_name in self.sorted_nicely(list(self.acpi.acpi_tables)):
            ssdt_name = "SSDT-PLUG"
            table = self.acpi.acpi_tables[table_name]
            if table.get("signature") not in (b"DSDT", b"SSDT"):
...
DefinitionBlock ("", "SSDT", 2, "ZPSS", "CpuPlugA", 0x00003000)
{
    ...
    {""".replace("[[parent]]", parent)
        ...
        Processor ([[name]], [[uid]], 0x00000510, 0x06)
        {
            ...
            return {"Add": [{"Comment": ssdt_name + ".aml", "Enabled": self.write_ssdt(ssdt_name, ssdt), "Path": ssdt_name + ".aml"}]}
    ...
    def fix_irq_conflicts(self):
        ...
        if target_irqs is None:
            return  # Bailed, going to the main menu
        # Let's apply patches as we go
        saved_dsdt = self.dsdt.get("raw")
        unique_patches = {}
        generic_patches = []
        for dev in devs:
            if dev not in target_irqs:
                continue
            ...
            for _a, t in enumerate(i):
                ...
                if dev not in unique_patches:
                    unique_patches[dev] = []
                unique_patches[dev].append(
                    {"dev": dev, "remd": ",".join([str(y) for y in set(t["remd"])]), "orig": t["find"], "find": t_patch, "repl": r_patch}
                )
        ...
        if len(generic_patches):
            ...
            for i, x in enumerate(generic_set):
                ...
                patches.append({"Comment": patch_name, "Find": x["find"], "Replace": x["repl"], "Enabled": False})
...
DefinitionBlock ("", "SSDT", 2, "ZPSS", "HPET", 0x00000000)
{
    ...
    Scope ([[name]])
    {
        ...
        return {
            "Add": [{"Comment": ssdt_name + ".aml", "Enabled": self.write_ssdt(ssdt_name, ssdt_content), "Path": ssdt_name + ".aml"}],
            "Patch": patches,
        }
...
"""
        for x in (awac_dict, rtc_dict):
            if not x.get("valid") or x.get("has_var") or not x.get("device"):
                continue
...
""".replace("[[DevPath]]", x["device"][0])
                    ...
                    .replace("[[macOS]]", macos)
                    .replace("[[sta_type]]", x["sta_type"])
                    .replace("[[called]]", " ()" if x["sta_type"] == "MethodObj" else "")
                )
...
""".replace("[[LPCName]]", lpc_name)
        ...
        if self.write_ssdt(ssdt_name, ssdt):
            return {
                "Add": [{"Comment": ssdt_name + ".aml", "Enabled": self.write_ssdt(ssdt_name, ssdt), "Path": ssdt_name + ".aml"}],
        ...
        def sta_needs_patching(sta):
            ...
            if sta.get("sta_type") == "IntObj":
                ...
                try:
                    ...
                    if "Name (_STA, 0x0F)" not in sta_scope:
                        return True
                except Exception:
                    # print(e)
                    return True
                ...
                try:
                    ...
                    if sta_scope.count("Return (") > 1 or "Return (0x0F)" not in sta_scope:
...
""".replace("[[LPCName]]", lpc_name)
                ...
                .replace(
                    "[[XSTA]]",
                    "{}.XSTA{}".format(x, " ()" if ec_sta[x].get("sta_type", "MethodObj") == "MethodObj" else "") if x in ec_sta else "0x0F",
                )
            )
...
""".replace("[[LPCName]]", lpc_name)
                ...
                .replace(
                    "[[XSTA]]",
                    "{}.XSTA{}".format(x, " ()" if ec_enable_sta[x].get("sta_type", "MethodObj") == "MethodObj" else "")
                    if x in ec_enable_sta
                    else "Zero",
                )
            )
    ...
    Scope ([[LPCName]])
    {
        ...
        return {"Add": [{"Comment": ssdt_name + ".aml", "Enabled": self.write_ssdt(ssdt_name, ssdt), "Path": ssdt_name + ".aml"}], "Patch": patches}
    ...
    def write_ssdt(self, ssdt_name, ssdt_content, _should_compile=True):
        ...
        output = self.run({"args": [self.acpi.iasl, dsl_path]})
...
DefinitionBlock ("", "SSDT", 2, "ZPSS", "IMEI", 0x00000000)
{
    ...
    Scope (_SB.PCI0)
    {
        ...
        if not imei_device:
            return {"Add": [{"Comment": ssdt_name + ".aml", "Enabled": self.write_ssdt(ssdt_name, ssdt_content), "Path": ssdt_name + ".aml"}]}
...
DefinitionBlock ("", "SSDT", 2, "ZPSS", "USBX", 0x00001000)
{
    Scope (\\_SB)
    {
        ...
        if usb_power_properties:
            ssdt_content = ssdt_content.replace(
                "[[USBX_PROPS]]",
                ",".join(
    ...
    Scope ([[ALSDevice]])
    {
        Method (_STA, 0, NotSerialized)  // _STA: Status
        {
            ...
            Else
            {
                ...
                .replace(
                    "[[XSTA]]", "{}.XSTA{}".format(als_device, " ()" if sta.get("sta_type", "MethodObj") == "MethodObj" else "") if sta else "0x0F"
                )
            )
        ...
        for device_name, device_props in self.disabled_devices.items():
            ...
            if "GPU" in device_name and device_props.get("Device Type") != "Integrated GPU":
                ...
                for _, table_data in self.acpi.acpi_tables.items():
                    ...
                    off_method_found = off_method_found or any(
                        method[0].startswith(target_device) and not self.is_method_in_power_resource(method, table_data.get("lines"))
                        for method in off_methods
                    )
...
"""
            ...
            if ssdt_name:
                ...
                results["Add"].append(
                    {"Comment": ssdt_name + ".aml", "Enabled": self.write_ssdt(ssdt_name, ssdt_content), "Path": ssdt_name + ".aml"}
                )
    ...
    def enable_backlight_controls(self):
        ...
        for table_name in self.sorted_nicely(list(self.acpi.acpi_tables)):
            ...
            if binascii.unhexlify("084E4243460A00") in table.get("raw"):
                patches.append({"Comment": "NBCF 0x00 to 0x01", "Find": "084E4243460A00", "Replace": "084E4243460A01"})
                break
            elif binascii.unhexlify("084E42434600") in table.get("raw"):
                patches.append({"Comment": "NBCF Zero to One", "Find": "084E42434600", "Replace": "084E42434601"})
                break
...
DefinitionBlock ("", "SSDT", 2, "ACDT", "PMCR", 0x00001000)
{
    ...
    def fix_hp_005_post_error(self):
        if binascii.unhexlify("4701700070000108") in self.dsdt.get("raw"):
            return {
                "Patch": [
                    {"Comment": "Fix HP Real-Time Clock Power Loss (005) Post Error", "Find": "4701700070000108", "Replace": "4701700070000102"}
                ]
            }
...
{"""
    ...
    def instant_wake_fix(self):
        ...
        if binascii.unhexlify(gprw_method) in self.dsdt.get("raw"):
            patches.append({"Comment": "GPRW to XPRW Rename", "Find": gprw_method, "Replace": xprw_method})
        else:
            gprw_method = None
        if binascii.unhexlify(uprw_method) in self.dsdt.get("raw"):
            patches.append({"Comment": "UPRW to XPRW Rename", "Find": uprw_method, "Replace": xprw_method})
        else:
            uprw_method = None
        if binascii.unhexlify(uswe_object) not in self.dsdt.get("raw"):
            uswe_object = None
        if binascii.unhexlify(wole_object) not in self.dsdt.get("raw"):
            wole_object = None
...
[[OSIStrings]]
        ...
        Else
        {
            ...
            "[[OSIStrings]]",
            "\n,".join(
                ['            "{}"'.format(osi_string) for target_os, osi_string in self.osi_strings.items() if osi_string in self.dsdt.get("table")]
            ),
        )
        ...
        if osif:
            patches.append({"Comment": "OSIF to XSIF rename - must come before _OSI to XOSI rename!", "Find": "4F534946", "Replace": "58534946"})
    ...
    def fix_apic_processor_id(self):
        ...
        for table_name in self.sorted_nicely(list(self.acpi.acpi_tables)):
            ...
            if processor_index != -1:
                return {
                    "Add": [{"Comment": "APIC.aml", "Enabled": self.write_ssdt("APIC", new_apic), "Path": "APIC.aml"}],
                    "Delete": [self.dropping_the_table("APIC")],
                }
...
"""
        for table_name in self.sorted_nicely(list(self.acpi.acpi_tables)):
            ...
            return {
                "Add": [{"Comment": ssdt_name + ".aml", "Enabled": self.write_ssdt(ssdt_name, ssdt_content), "Path": ssdt_name + ".aml"}],
                "Patch": [{"Comment": "WQBI to XQBI Rename", "Find": "5751424901", "Replace": "5851424901"}],
            }
    ...
    def select_acpi_patches(self, hardware_report, disabled_devices):
        ...
        if (
            "Laptop" in hardware_report.get("Motherboard").get("Platform")
            and "Integrated GPU" in list(hardware_report.get("GPU").items())[-1][-1].get("Device Type")
            and "SURFACE" not in hardware_report.get("Motherboard").get("Name")
        ):
        ...
        for _, device_info in hardware_report.get("System Devices", {}).items():
            ...
            if (
                "Sandy Bridge" in hardware_report.get("CPU").get("Codename")
                and device_id in "8086-1E3A"
                or "Ivy Bridge" in hardware_report.get("CPU").get("Codename")
                and device_id in "8086-1C3A"
            ):
        ...
        if (
            ...
            or hardware_report.get("Motherboard").get("Chipset") in chipset_data.IntelChipsets[112:]
        ):
