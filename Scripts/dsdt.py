...
from Scripts import github, resource_fetcher, run, utils
...
class DSDT:
    def __init__(self, **kwargs):
        ...
        if not self.iasl:
            url = (
                ...
                if os.name == "nt"
                else self.iasl_url_macOS
                if sys.platform == "darwin"
                else self.iasl_url_linux
    ...
    def load(self, table_path):
        ...
        try:
            ...
            if not valid_files:
                ...
                raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), "No valid .aml/.dat files found at {}".format(table_path))
            # Create a temp dir and copy all files there
            temp = tempfile.mkdtemp()
            for file in valid_files:
                shutil.copy(os.path.join(table_path, file), temp)
    ...
    def get_unique_pad(self, current_hex, index, direction=None, instance=0, table=None):
        if not table:
            table = self.get_dsdt_or_only()
        if not table:
            raise Exception("No valid table passed!")
