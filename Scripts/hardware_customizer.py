from Scripts import compatibility_checker, utils
...
class HardwareCustomizer:
    ...
    def hardware_customization(self, hardware_report, macos_version):
        ...
        for device_type, devices in self.hardware_report.items():
            if device_type not in ("BIOS", "GPU", "Sound", "Biometric", "Network", "Storage Controllers", "Bluetooth", "SD Controller"):
            ...
            for device_name in devices:
                ...
                if device_props.get("OCLP Compatibility") and self.utils.parse_darwin_version(
                    device_props.get("OCLP Compatibility")[0]
                ) >= self.utils.parse_darwin_version(macos_version) >= self.utils.parse_darwin_version(device_props.get("OCLP Compatibility")[-1]):
                ...
                try:
                    if (
                        self.utils.parse_darwin_version(device_compatibility[0])
                    ...
                    self.disabled_devices[
                        "{}: {}{}".format(
                            ...
                            device_name,
                            "" if not device_props.get("Audio Endpoints") else " ({})".format(", ".join(device_props.get("Audio Endpoints"))),
                        )
                    ] = device_props

                if self.customized_hardware[device_type].get(device_name) and self.customized_hardware[device_type][device_name].get(
                    "OCLP Compatibility"
                ):
        ...
        if len(devices) > 1:
            ...
            if selected_devices:
                for selected_device in selected_devices:
                    if device_type not in self.selected_devices:
                ...
                if len(group_devices) == 1:
                    ...
                    if device_props.get("OCLP Compatibility"):
                        ...
                        if self.utils.parse_darwin_version(oclp_compatibility[0]) > self.utils.parse_darwin_version(group_compatibility[0]):
                            print(
                                "   OCLP Compatibility: {}".format(
                                    self.compatibility_checker.show_macos_compatibility((oclp_compatibility[0], os_data.get_lowest_darwin_version()))
                                )
                            )
                print("")
